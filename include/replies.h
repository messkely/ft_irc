...
#define ERR_UNKNOWNCOMMAND(command)						"421 " + std::string(command) + " :Unknown command" + LF 
...
#define ERR_NEEDMOREPARAMS(command)						"461 " + std::string(command) + " :Syntax error" + LF
#define ERR_ALREADYREGISTRED(command)					"462 " + std::string(command) + " :Unauthorized command (already registered)" LF
#define ERR_PASSWDMISMATCH(target)						"464 " + target + " :Password incorrect" LF
...
// Replies :
#define RPL_WELCOME(nick, user, host) 					": 001 " + nick + " :Welcome to the IRC network, " + nick + "!" + user + "@" + host + LF
