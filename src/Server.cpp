	...
	: servSock(getServSock(port)), passwd(passwd)
{
	...
	string	tmpCmdNames[CMDS_N] = {PASS, NICK, USER, JOIN, MODE, PART, TOPIC, KICK}; // add command names here

	cmdCreator	tmpCmdFactory[CMDS_N] =
	{Pass::create, Nick::create, User::create, Join::create, Mode::create, Part::create, Topic::create, Kick::create, Invite::create, Privmsg::create}; // add factory methods here
...
void	handleUnknownCommand(Client &client, string msg)
{
	...
	client << ERR_UNKNOWNCOMMAND((std::string)args[0]);
}
