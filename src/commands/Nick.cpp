...
void	Nick::parse()
{
	...
	if (argc < NICK_ARGS_N)
	{
		respStr = ERR_NEEDMOREPARAMS((std::string)NICK);
		return ;
	}
	...
	if (argc > NICK_ARGS_N || isNickErroneous(args[1]))
	{
		...
		respStr = ERR_ERRONEUSNICKNAME((std::string)NICK);
		return ;
	}

	if (client.getNickname().empty() && server.isNicknameTaken(args[1])) // first nick ever
	{
		respStr = ERR_NICKCOLLISION((std::string)args[1]);
		return ;
	}

	if (server.isNicknameTaken(args[1]))
	{
		respStr = ERR_NICKNAMEINUSE((std::string)args[1]);
		return ;
	}
}
...
void	Nick::execute()
{
	...
	if (!client.getUsername().empty() && client.getNickname().empty())
		respStr = RPL_WELCOME((std::string)args[1], client.getUsername(), client.getHostname());
